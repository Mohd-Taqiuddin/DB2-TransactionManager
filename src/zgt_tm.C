...
//modified by sharma in oct 2022 to fix handling with end all as last input + a few others

//Last Modified at 6:34 PM 09/29/2016 by Jay D. Bodra. 
...
int zgt_tm::TxWrite(long tid, long obno, int thrNum)
 {
  //call the write function (writetx); same as above
   ...
   fflush(stdout);
      ...
      nodeinfo->count = --SEQNUM[tid];
      int status;
      status = pthread_create(&threadid[thrNum],NULL,writetx,(void*)nodeinfo);// Fork a thread with tid
      if (status){
      printf("ERROR: return code from pthread_create() is:%d\n", status);
      exit(-1);
      }
   ...
   int zgt_tm::CommitTx(long tid, int thrNum)
   {
      ...
      nodeinfo->count = --SEQNUM[tid];
      int status;
      status = pthread_create(&threadid[thrNum],NULL,committx,(void*)nodeinfo);// Fork a thread with tid
      if (status){
      printf("ERROR: return code from pthread_create() is:%d\n", status);
      exit(-1);
      }
